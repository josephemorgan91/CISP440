...
//
//	2. Create an empty logical matrix to store equiv. classes to be
//generated.
//
// 	3. For each row r in the relation matrix,
// 		**	TODO: It just ocurred to me that to make the generated
// matrix more
// 			random, r shouldn't iterate from 0 to size, but be a random
// row until
// 			each row is processed... Equiv. classes seem to be weighted
// toward the
// 			earlier rows. I probably won't get around to fixing that
// though...
//
// 		a. Check each of your equiv. classes for a row with r. For
// example,
// 				if you're looking at matrix[3][x], you need to add
// matrix[3][3] to
// 				that row. You don't want to add the 3 if it already
// exists in an 				equiv. class, so you scan
// through all classes to make sure it's not 				there.
//
// 			i. If an equiv. class exists with column r, copy that class to
// row r
// 					of the relation matrix. Now you've got matrix[r][r]
// to satisfy
// 					reflexivity, and you haven't broken transitivity by
// overlapping an 					equiv. class.
//
// 			ii. If no equiv. class exists with r, randomly generate one,
// being 					careful to make sure you do not
// ovelap any values already in 					an
// equiv. class. Manually insert r value. Copy to relation matrix.
//
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (strcmp(argv[i], "-s") == 0) {
      ...
      fprintf(stderr, "Usage: %s [-s size] [-e density] [-v] [-d] [-h]\n",
              argv[0]);
      fprintf(stderr,
              "    -s size: Size of the matrix to be randomly generated\n");
      fprintf(stderr, "    -e density: Likelyhood that any given relation will "
                      "exist. Higher means less likely\n");
  ...
  if (verbose) {
    ...
    int is_EQR = 1;
    printf("\n");
    is_reflexive(test_rand) ? (printf("Is Reflexive\n"))
                            : (is_EQR = 0, printf("Isn't Reflexive\n"));
    is_symetric(test_rand) ? (printf("Is Symetrical\n"))
                           : (is_EQR = 0, printf("Isn't Symetrical\n"));
    is_transitive(test_rand) ? (printf("Is Transitive\n"))
